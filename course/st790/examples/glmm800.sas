/*-----------------------------------------------------------
 ...
 improved vertical bar processing as suggested
    ...
    Schabenberger, Va Tech                      26Apr02   rdw
 added terms to deviance calculation for Poisson 
    ...
    uses the previous formulas                  06Jun02   os
 ...
 where                                                        
  ...
  zmult    specifies a variable by which the pseudo data variable
           _z is multiplied during each iteration.
                                                              
  wadd     specifies a variable to add to the weight variable _w
  ...
  options  specifies GLIMMIX macro options separated by       
     ...
     OLDDEVIANCE     requests computation of deviances for poisson and 
...
%macro glimmix(data=,procopt=,stmts=,weight=,freq=,
    ...
    converge=1e-8,maxit=20,offset=0,zmult=,wadd=,out=,outalpha=0.05,
    options=);
 ...
 /*---change prediction method from mean to eblup---*/
 %if (%index(&options,MQI) and (&conv = 1)) %then %do;
    ...
    data _pred1;
       set _pred;
       keep pred stderrpred lower upper;
    run;
    data _ds;
       set _ds;
       drop pred stderrpred lower upper;
    run;
    data _ds;
       merge _ds _pred1;
    run;
 ...
 %if (&exiterr = 1) %then %do;
    %if not %index(&options,NOPRINT) %then %do;
       %put GLIMMIX exited due to errors.;
    %end;
 %end;
