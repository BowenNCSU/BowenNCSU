...
USAGE:
   ...
   Follow the instructions in the Downloads tab of this sample to
   save the %NLINMIX macro definition. Replace the text within quotes in
   the following statement with the location of the %NLINMIX macro
   definition file on your system. In your SAS program or in the SAS
   editor window, specify this statement to define the %NLINMIX macro and
...
DETAILS:
   ...
   Available estimation methods are as follows, all of
   which are implemented via iterative calls to PROC MIXED using
   appropriately constructed pseudo data:

        Expanding the nonlinear function about random effects
   parameters set equal to zero, which is similar to but not the same as
   Sheiner and Beal's first-order method (Beal and Sheiner, 1982). This
   method is the default for random effects specifications; that is,
   those using RANDOM statements in PROC MIXED.

        Expanding the nonlinear function about random effects
   parameters set equal to their current empirical best linear unbiased
   predictor (EBLUP), which is Lindstrom and Bates' approximate
   second-order method (Lindstrom and Bates, 1990). The method is
   implemented using an initial call to PROC NLIN and then iterative
   calls to PROC MIXED. This differs from Lindstrom and Bates'
   implementation, in which pseudo data are constructed and iterative
   calls are made alternately to PROC MIXED and PROC NLIN. This macro's
   implementation requires much less time and space for larger problems,
   and it works because solving the mixed-model equations in PROC MIXED
   is equivalent to taking the first Gauss-Newton step in PROC NLIN
   (Wolfinger, 1993). Use EXPAND=EBLUP to implement this method, which
   applies for RANDOM statement specifications in PROC MIXED.

        Iteratively fitting a covariance structure outside of the
   nonlinear function. This solves a type of second-order generalized
   estimating equations (Prentice and Zhao, 1991). This method uses the
   REPEATED statement in PROC MIXED.

    The %NLINMIX macro displays an iteration history to the log (check
   this to verify convergence) and then displays the results of the final
   call to PROC MIXED. The fixed- and random-effect parameter estimates
   are prefixed by "d_" in the results.
